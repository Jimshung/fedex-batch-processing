...
- **ETD 功能** - 電子貿易文件（商業發票）✅
- **訂單識別機制** - 使用 `customerTransactionId` 關聯 Shopify 訂單 ✅

#### 🎯 最新成功測試結果

**Postman 測試成功：**

- ✅ **API 端點**: `https://apis.fedex.com/ship/v1/shipments`
- ✅ **回應狀態**: 200 OK
- ✅ **追蹤號碼**: `883773277341`
- ✅ **交易識別碼**: `45a3043f-6010-4c18-a1e7-d0e662f64b09`
- ✅ **商業發票**: 成功生成 PDF 文件
...
```json
{
  "customerTransactionId": "90084", // Shopify 訂單編號
  "transactionId": "45a3043f-6010-4c18-a1e7-d0e662f64b09", // FedEx API 交易識別碼
  "masterTrackingNumber": "883773277341" // FedEx 追蹤號碼
}
```
...
- 收件人資訊：從 Shopify 訂單動態取得
- 商品計算：根據訂單數量動態計算重量和價值
- 地址處理：自動分割長地址符合 FedEx 要求
- 海關申報：動態計算單價和總價值
...
- 系統會顯示 `SHIPMENT.SHIPDATESTAMP.INVALID` 警告
- 這是因為測試日期設定為過去日期
- 生產環境使用當前日期，不會有此警告
...
- 圖片上傳端點在測試環境不可用
- 生產環境使用固定的 `IMAGE_1` 和 `IMAGE_2` 引用
- 不影響核心運單建立功能
